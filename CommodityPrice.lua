...
local function B(k)
  ...
  function E:setCurrentPriceColor(V)
    local p=false;
    if self.currentPriceColor~=V then 
      self.currentPriceColor=V;
      p=true 
    end;
    return p 
  end;
  function E:calculateCurrentPriceColor()
    local V;
    if self:getIsTrendingUp() then
      V={getColorEx('green')}
    elseif self:getIsTrendingDown() then
      V={getColorEx('red')}
    else V={getColorEx('white')}
    end;
    self:setCurrentPriceColor(V)
  end;
  function E:setDeviationPercentColor(W)
    local p=false;
    if self.deviationPercentColor~=W then 
      self.deviationPercentColor=W;
      p=true 
    end;
    return p
  end;
  function E:calculateDeviationPercentColor()
    local W;
    if self.deviationPercent<0 then 
      W={getColorEx('red')}
    elseif self.deviationPercent>0 then
      W={getColorEx('green')}
    else 
      W={getColorEx('white')}
    end;
    self:setDeviationPercentColor(W)
  end;
  return E
end;
local function X(Y)
  ...
  function Y:parseX(Z)
    local _='l'
    local a0=nil;
    local a1=self.ParentAbsoluteWidth or 1;
    if type(Z)=='string'then
      Z=Z:gsub("^%s*(.-)%s*$","%1")
      _=Z:sub(-1):lower()
      if _=='r'or _=='c'or _=='l'then
        Z=Z:sub(1,Z:len()-1):gsub("^%s*(.-)%s*$","%1")
      else 
        _='l'
      end;
      if Z:sub(-1)=='%'then 
        Z=Z:sub(1,Z:len()-1):gsub("^%s*(.-)%s*$","%1")
        a0=a1*tonumber(Z)/100
      elseif Z:sub(-2):lower()=='px'then
        Z=Z:sub(1,Z:len()-2):gsub("^%s*(.-)%s*$","%1")
        a0=self:pxToNormal(tonumber(Z),'x')else a0=tonumber(Z)
      end 
    elseif Z~=nil then
      a0=tonumber(Z)
    end;
  ...
  function Y:parseWidth(Z)
    local a2=nil;
    if type(Z)=='string'then
      Z=Z:gsub("^%s*(.-)%s*$","%1")
      if Z:sub(-1)=='%'then 
        Z=Z:sub(1,Z:len()-1):gsub("^%s*(.-)%s*$","%1")
        a2=(self.ParentAbsoluteWidth or 1)*tonumber(Z)/100 
      elseif Z:sub(-2):lower()=='px'then
        Z=Z:sub(1,Z:len()-2):gsub("^%s*(.-)%s*$","%1")
        a2=self:pxToNormal(tonumber(Z),'x')
      else 
        a2=tonumber(Z)
      end 
    elseif Z~=nil then 
  ...
  function Y:parseY(Z)
    ...
    local a3=nil;
    local a4=Z;
    local a5=self.ParentAbsoluteHeight or 1;
    if type(Z)=='string'then 
      Z=Z:gsub("^%s*(.-)%s*$","%1")
      _=Z:sub(-1):lower()
      if _=='t'or _=='m'or _=='b'then
        Z=Z:sub(1,Z:len()-1):gsub("^%s*(.-)%s*$","%1")
      else
        _='b'
      end;
      if Z:sub(-1)=='%'then 
        Z=Z:sub(1,Z:len()-1):gsub("^%s*(.-)%s*$","%1")
        a3=a5*tonumber(Z)/100 
      elseif Z:sub(-2):lower()=='px'then 
        Z=Z:sub(1,Z:len()-2):gsub("^%s*(.-)%s*$","%1")
        a3=self:pxToNormal(tonumber(Z),'y')
      else 
        a3=tonumber(Z)
      end 
    elseif Z~=nil then 
  ...
  function Y:parseHeight(Z)
    local a6=nil;
    if type(Z)=='string'then 
      Z=Z:gsub("^%s*(.-)%s*$","%1")
      if Z:sub(-1)=='%'then
        Z=Z:sub(1,Z:len()-1):gsub("^%s*(.-)%s*$","%1")
        a6=(self.ParentAbsoluteHeight or 1)*tonumber(Z)/100 
      elseif Z:sub(-2):lower()=='px'then
        Z=Z:sub(1,Z:len()-2):gsub("^%s*(.-)%s*$","%1")
        a6=self:pxToNormal(tonumber(Z),'y')
      else
        a6=tonumber(Z)
      end 
    elseif Z~=nil then
  ...
  local a8=1080;
  local a9=a7/a8;
  local aa=g_screenAspectRatio/a9;
  local ab=1;
  if g_screenWidth>a7 then
    ab=a7/g_screenWidth end;
  function Y:pxToNormal(ac,ad,ae)
    local af;
    if ad=='x'then 
      af=ac/a7*ab 
    else 
    ...
    if ae==nil or ae then
      af=self:getFullPx(af,ad)
    end;
    return af
  end;
  function Y:getFullPx(ag,ad)
    if ad=='x'then 
      return C(ag*g_screenWidth)/g_screenWidth 
    else 
      return C(ag*g_screenHeight)/g_screenHeight 
    end
  end;
  function Y:getPxToNormalConstant(ah,ai)
    return ah/g_screenWidth,ai/g_screenHeight 
  end;
  function Y:getPxToNormalConstant(ah,ai)
    return ah/g_screenWidth,ai/g_screenHeight 
  end
...
local function aj(Y)
  Y.sorts={}
  Y.sorts["displayName"]=function(self,g,f)
    if g.displayName==f.displayName then
      return 0 
    elseif g.displayName<f.displayName then 
      return 1 
    else
      return -1
    end 
  end;
  Y.sorts["currentPriceLocationDisplayName"]=function(self,g,f)
    if g.currentPriceLocationDisplayName==f.currentPriceLocationDisplayName then 
      return 0 
    elseif g.currentPriceLocationDisplayName<f.currentPriceLocationDisplayName then 
      return 1
    else 
      return -1 
    end 
  end;
  Y.sorts["lowestPrice"]=function(self,g,f)
    local ak;
    local al;
    if self.player.normalizePrices then
      ak=g.lowestPriceNormalized
    else 
      ak=g.lowestPrice 
    end;
    if self.player.normalizePrices then 
      al=f.lowestPriceNormalized 
    else 
      al=f.lowestPrice 
    end;
    if ak==al then
  ...
  Y.sorts["meanPrice"]=function(self,g,f)
    local am;
    local an;
    if self.player.normalizePrices then 
      am=g.meanPriceNormalized 
    else
      am=g.meanPrice 
    end;
    if self.player.normalizePrices then
      an=f.meanPriceNormalized
    else 
      an=f.meanPrice 
    end;
    if am==an then 
  ...
  Y.sorts["highestPrice"]=function(self,g,f)
    local ao;
    local ap;
    if self.player.normalizePrices then 
      ao=g.highestPriceNormalized 
    else ao=g.highestPrice end;
    if self.player.normalizePrices then
      ap=f.highestPriceNormalized 
    else ap=f.highestPrice 
    end;
    if ao==ap then 
      return 0 
    elseif ao>ap then
  ...
  Y.sorts["currentPrice"]=function(self,g,f)
    local aq;
    local ar;
    if self.player.normalizePrices then
      aq=g.currentPriceNormalized 
    else aq=g.currentPrice
    end;
    if self.player.normalizePrices then
      ar=f.currentPriceNormalized 
    else 
      ar=f.currentPrice 
    end;
    if aq==ar then
  ...
  Y.sorts["deviationPercent"]=function(self,g,f)
    if g.deviationPercent==f.deviationPercent then 
      return 0 
    elseif g.deviationPercent>f.deviationPercent then 
      return 1 
    else 
      return -1
    end 
  end;
  Y.columnOrders={{"displayName"},{"currentPriceLocationDisplayName","displayName"},{"lowestPrice","deviationPercent","displayName"},{"meanPrice","deviationPercent","displayName"},{"highestPrice","deviationPercent","displayName"},{"currentPrice","deviationPercent","displayName"},{"deviationPercent","currentPrice"}}
end;
local function as(Y)
  function Y:mouseEvent(at,au,av,aw,ax)
  end;
  function Y:keyEvent(ay,az,aA,av)
    if az==Input.KEY_lshift then 
      self.rightControlDown=av 
    end;
    if az==Input.KEY_ralt then 
      self.rightAltDown=av 
    end;
    if az==Input.KEY_rshift then 
      self.rightShiftDown=av 
    end;
    if az==Input.KEY_rctrl and av and(aA==0 or self.rightControlDown)then 
      self.accessKeyDownDt=self.dt;
      if self.isVisibleState==0 then 
        self.isVisibleState=1 
      end 
    elseif az==Input.KEY_rctrl and not av and self.accessKeyDownDt>0 then 
      if self.isVisibleState==1 then 
        if self.dt-self.accessKeyDownDt<=500 then 
          self.isVisibleState=2 
        else 
          self.isVisibleState=0 
        end 
      elseif self.isVisibleState==2 and self.dt-self.accessKeyDownDt<=500 and not self.otherFunctionClicked then 
        self.isVisibleState=0 
      end;
      self.accessKeyDownDt=0;
      InputBinding.setShowMouseCursor(false)
      self.otherFunctionClicked=false 
    elseif az==Input.KEY_delete and self.accessKeyDownDt>0 and av then 
      self:setNormalizePrices(not self.player.normalizePrices)
      self.otherFunctionClicked=true 
    elseif az==Input.KEY_insert and self.accessKeyDownDt>0 and av then 
      if self.player.orderBy>=7 then 
        self:setOrderBy(1)
      else self:setOrderBy(self.player.orderBy+1)
      end;
      self.otherFunctionClicked=true 
    elseif az==Input.KEY_right and self.accessKeyDownDt>0 and av then
      if self.rightControlDown then
        self:setHAlignment("right")
      elseif self.rightShiftDown then
        self:setBasePosX(self.player.basePosX+self:parseWidth('1px'))
      else 
        self:setBasePosX(self.player.basePosX+self:parseWidth('10px'))
      end;
      self.otherFunctionClicked=true 
    elseif az==Input.KEY_left and self.accessKeyDownDt>0 and av then
      if self.rightControlDown then 
        self:setHAlignment("left")
      elseif self.rightShiftDown then
        self:setBasePosX(self.player.basePosX-self:parseWidth('1px'))
      else self:setBasePosX(self.player.basePosX-self:parseWidth('10px'))
      end;
      self.otherFunctionClicked=true 
    elseif az==Input.KEY_up and self.accessKeyDownDt>0 and av then 
      if self.rightControlDown then
        self:setVAlignment("top")
      elseif self.rightShiftDown then
        self:setBasePosY(self.player.basePosY+self:parseHeight('1px'))
      else 
        self:setBasePosY(self.player.basePosY+self:parseHeight('10px'))
      end;
      self.otherFunctionClicked=true 
    elseif az==Input.KEY_down and self.accessKeyDownDt>0 and av then
      if self.rightControlDown then 
        self:setVAlignment("bottom")
      elseif self.rightShiftDown then 
        self:setBasePosY(self.player.basePosY-self:parseHeight('1px'))
      else 
        self:setBasePosY(self.player.basePosY-self:parseHeight('10px'))
      ...
      self.otherFunctionClicked=true 
    elseif az==Input.KEY_pageup and self.accessKeyDownDt>0 and av then
      if self.rightControlDown then 
        self:setBackgroundAlpha(self.player.backgroundAlpha+0.05)
      else self:setFontSize(self.player.fontSize+self:parseHeight('1px'))
      end;
      self.otherFunctionClicked=true
    elseif az==Input.KEY_pagedown and self.accessKeyDownDt>0 and av then 
      if self.rightControlDown then 
        self:setBackgroundAlpha(self.player.backgroundAlpha-0.05)
      else if self.player.fontSize>self:parseHeight('10px')then
          self:setFontSize(self.player.fontSize-self:parseHeight('1px'))
        end 
      end;
      self.otherFunctionClicked=true
    end
...
local function aB(Y)
  ...
  Y.isVisibleState=2;
  function Y:uiInit()
    self:setFontSize(0.01)
    self:setBasePosX(0.97708)
    self:setBasePosY(0.89583)
    self:setHAlignment("right")
    self:setVAlignment("top")
    self:setBackgroundAlpha(0.7)
    self:setNormalizePrices(true)
    self:setOrderBy(7)
  end;
  function Y:setNormalizePrices(aC)
    if self.player:setNormalizePrices(aC)then 
      self.commoditiesDirty=true
    end
  end;
  function Y:setBackgroundAlpha(aD)
    if aD<0 then
      aD=0 
    elseif aD>1 then 
      aD=1 
    end;
    if self.player:setBackgroundAlpha(aD)then 
      self:calculateOverlayColor()
    end 
  end;
  function Y:setBasePosX(at)
    if self.player:setBasePosX(at)then
      self:calculateAbsoluteX()
    end 
  end;
  function Y:setHAlignment(aE)
    if self.player:setHAlignment(aE)then 
      self:calculateAbsoluteX()
    end
  end;
  function Y:calculateAbsoluteX()
    local aF=self.player.basePosX;
    if self.player.hAlignment=="right"then 
      aF=aF-self.width 
    end;
    if self.absolutePosX~=aF then 
      self.absolutePosX=aF;
      self:calculateOverlay()
    end 
  end;
  function Y:setBasePosY(au)
    if self.player:setBasePosY(au)then 
      self:calculateAbsoluteY()
    end 
  end;
  function Y:setVAlignment(aG)
    if self.player:setVAlignment(aG)then 
      self:calculateAbsoluteY()
    end
  end;
  function Y:calculateAbsoluteY()
    local aH=self.player.basePosY;
    if self.player.vAlignment=="top"then
      aH=aH-self.height end;
    if self.absolutePosY~=aH then 
      self.absolutePosY=aH;self:calculateOverlay()
    end
  end;
  function Y:setFontSize(aI)
    if self.player:setFontSize(aI)then
      self:setOverlayPaddingX(getTextWidth(self.player.fontSize,'M')*0.5)
      self:setOverlayPaddingY(getTextHeight(self.player.fontSize,'M')*0.5)
      self:setColumnPaddingWidth(getTextWidth(self.player.fontSize,'   '))
      self:setLineHeight(getTextHeight(self.player.fontSize,'Wy'))
      self:calculateColumnWidths()
    end
  end;
  function Y:setOverlayPaddingX(aJ)
    if self.overlayPaddingX~=aJ then 
      self.overlayPaddingX=aJ;
      self:calculateOverlay()
    end
  end;
  function Y:setOverlayPaddingY(aK)
    if self.overlayPaddingY~=aK then 
      self.overlayPaddingY=aK;
      self:calculateOverlay()
    end
  end;
  function Y:setLineHeight(aL)
    if self.lineHeight~=aL then 
      self.lineHeight=aL;
      self:calculateHeight()
    end
  end;
  function Y:setColumnPaddingWidth(aM)
    if self.columnPaddingWidth~=aM then 
      self.columnPaddingWidth=aM;
      self:calculateWidth()
    end
  end;
  function Y:calculateWidth()
    local a2=0;
    for aN,aO in ipairs(self.columns)do 
      a2=a2+(aO.width or 0)
    end;
    a2=a2+self.columnPaddingWidth*(#self.columns-1)
    if self.width~=a2 then 
      self.width=a2;
      self:calculateAbsoluteX()
      self:calculateOverlay()
    end 
  end;
  function Y:calculateHeight()
    local a6=self.lineHeight*(#self.sortedCommodities+1)
    if self.height~=a6 then
      self.height=a6;self:calculateAbsoluteY()
      self:calculateOverlay()
    end
  end;
  function Y:resetOverlay()
    if self.overlay~=nil then 
      self.overlay:delete()
      self.overlay=nil 
    end
  end;
  function Y:setOrderBy(aP)
    if self.player:setOrderBy(aP)then 
      self.commoditiesDirty=true 
    end
  end;
  function Y:calculateSortedCommodities()
    local aQ=null;
    if self.sortedCommodities~=nil then
      aQ=#self.sortedCommodities
    end;
    self.sortedCommodities={}
    for m,k in pairs(self.commodities)do 
      table.insert(self.sortedCommodities,k)
    end;
    table.sort(self.sortedCommodities,function(g,f))--MAY CAUSE PROBLEM
        for aN,aR in ipairs(self.columnOrders[self.player.orderBy])do 
          local aS=self.sorts[aR](self,g,f)
          if aS~=0 then 
            return aS<0
          end
        end;
    ...
    if aQ~=#self.sortedCommodities then 
      self:calculateHeight()
    end 
  end;
  function Y:calculateColumnWidths()
    setTextBold(true)
    for aN,aO in ipairs(self.columns)do
      aO.width=getTextWidth(self.player.fontSize,aO.title)
    end;
    setTextBold(false)
    for aN,k in ipairs(self.sortedCommodities)do 
      for aN,aO in ipairs(self.columns)do
        local a2=getTextWidth(self.player.fontSize,aO.displayValue(k))
        if a2>aO.width then 
          aO.width=a2 
        end
      end 
  ...
  function Y:calculateOverlay()
    if self.overlay~=nil then 
      self:resetOverlay()
    end;
    if self.absolutePosX~=nil and self.overlayPaddingX~=nil and self.absolutePosY~=nil and self.overlayPaddingY~=nil and self.width~=nil and self.height~=nil and self.player.backgroundAlpha~=nil then 
      self.overlay=Overlay:new('CommodityPricesOverlay',self.path..'img/white.dds',self.absolutePosX-self.overlayPaddingX,self.absolutePosY-self.overlayPaddingY,self.width+self.overlayPaddingX*2,self.height+self.overlayPaddingY*1.25)
      setOverlayUVs(self.overlay.overlayId,0,0,0,1,1,0,1,1)
      self:calculateOverlayColor()
    end 
  end;
  function Y:calculateOverlayColor()
    if self.overlay~=nil and self.player.backgroundAlpha~=nil then 
      self.overlay:setColor(0.025,0.025,0.025,self.player.backgroundAlpha)
    end 
  end;
  function Y:draw()
    if self.isVisibleState>0 then
      if self.commoditiesDirty then 
        self:calculateSortedCommodities()
        self:calculateColumnWidths()
        self.commoditiesDirty=false 
      end;
      if self.overlay~=nil then
        self.overlay:render()
      end;
      setTextBold(false)
      local aT=0;
      local aU=self.absolutePosY;
      for aN,k in ipairs(self.sortedCommodities)do 
        local aV=self.absolutePosX;
        for aW,aO in ipairs(self.columns)do
          if aW>1 then 
            aV=aV+self.columnPaddingWidth end;
          setTextColor(unpack(aO.color(k)))
          setTextAlignment(aO.alignment)
          if aO.alignment==RenderText.ALIGN_LEFT then
            renderText(aV,aU,self.player.fontSize,aO.displayValue(k))
          else 
            renderText(aV+aO.width,aU,self.player.fontSize,aO.displayValue(k))
          end;
          aV=aV+aO.width 
        end;
        aT=aT+1;
        aU=aU+self.lineHeight 
      end;
      setTextBold(true)
      local aV=self.absolutePosX;
      for aW,aO in ipairs(self.columns)do
        if aW>1 then 
          aV=aV+self.columnPaddingWidth
        end;
        if aW==self.player.orderBy then
          setTextColor(getColorEx('blue'))
        else 
          setTextColor(getColorEx('white'))
        end;
        setTextAlignment(aO.alignment)
        if aO.alignment==RenderText.ALIGN_LEFT then 
          renderText(aV,aU,self.player.fontSize,aO.title)
        else renderText(aV+aO.width,aU,self.player.fontSize,aO.title)
        end;
        aV=aV+aO.width
      end;
      aT=aT+1;
